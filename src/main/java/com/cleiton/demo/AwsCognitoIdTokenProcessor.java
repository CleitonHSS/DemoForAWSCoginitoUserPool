...
public class AwsCognitoIdTokenProcessor {
    ...
    private String getUserNameFrom(JWTClaimsSet claims) {
        String nameKey = this.jwtConfiguration.getUserNameField();
        String userName = claims.getClaims().get(nameKey).toString();
        return userName;
    }

    private void verifyToken(JWTClaimsSet claims) throws Exception {
        if (!claims.getIssuer().equals(this.jwtConfiguration.getCognitoIdentityPoolUrl())) {
            throw new Exception("JWT Token is not a valid Token");
        }
    }

    private void issuerValidate(JWTClaimsSet claims) throws Exception {
        if (!claims.getIssuer().equals(this.jwtConfiguration.getCognitoIdentityPoolUrl())) {
